...
def customer_text(text):
    """Remove private reasoning blocks if a model emits them as plain text."""
...
def invoke(rt, config, session_id, user_text, verbose=False):
    ...
    response = rt.invoke_harness(
        ...
        # Managed harness memory includes cross-session retrieval by default.
        # A unique actor per chat keeps this conversation isolated while the
        # reused session id still preserves its own multi-turn history.
        actorId=session_id,
    ...
    for event in event_stream(response):
        ...
        if "contentBlockStart" in event:
            ...
            if buffer:
                visible = customer_text("".join(buffer))
                if visible:
